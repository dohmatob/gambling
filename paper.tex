...
Developing on a homologuous and alternative notion of approximate equilibria
, we device a primal-dual algorithm (Algorithm \ref{Tab:algo}) for
...
prove that the number of iterations required by the algorithm to
...
\section{A more amenable notion of approximate equilibrium}
\subsection{Interlude: Proximal calculus}
Let us introduce some notions in modern convex analysis which will be
...
\begin{figure}[!htpb]
  \subfigure[Matrix game on simplexes]{
    \includegraphics[width=.33\linewidth]{0.pdf}
  }
  \hspace{-2em}
  \subfigure[Toy Poker]{
  ...
  \vspace{-1em}
  \hspace{-1em}
  \includegraphics[width=.34\linewidth]{1.pdf}
  \hspace{-1em}
...
\subsection{Sequential games with imcomplete information}
...
\paragraph{{Kuhn 3-card Poker}} is a simplified form of poker
developed by Harold W. Kuhn. It is a zero sum two player game. The
...
highest card wins the pot. Kuhn demonstrated that there are many game
theoretic optimal strategies for the first player in this game, but
only one for the second player, and that, when played optimally, the
first player should expect to lose at a rate of −1/18 per hand. The
sequence-form representation of the game has $n_1 = n_2 = 13$
sequences and $l_1 = l_2 = 7$ information
sets per player. More details (payoff matrix, etc.) are provided in
the supp. mats. %% , and its
...
Nash $(\epsilon,0)$-equilibrium of the game.
