...
%$\{x \in \mathbb{R}_{+}^m\text{ }| \text{ }\textbf{1}^Tx = 1\}$
}\\
...
%% \footnote{Note that the origin $\textbf{0} \in \mathbb{R}^m$ can be seen a degenerate simplex with radius $0^+$,
%% whilst a singleton  $\{y_0\}$ can be seen as a $y_0-$translation of this simplex.}
$\{y_0\}$.\\\\
...
\begin{equation}
  ...
  \begin{aligned}
    ...
    (1 + \sigma \partial F^*)^{-1}: (y, \theta) \mapsto (\Pi_{\Delta_{m}^0}(y), \theta + \sigma e)% (z, \theta) \mapsto \bigl(y, \frac{\theta}{1 + \sigma}\bigr)
  \end{aligned}
  \right\}
\end{equation}
...
\Repeat{
  ...
  \begin{eqnarray*}
    ...
    \theta^{(k+1)} &\leftarrow& \theta^{(k)} - \sigma E \tilde{x}^{(k)} + \sigma e\\
    x^{(k+1)} &\leftarrow& \bigl(x^{(k)} - \tau A^Ty^{(k+1)} + E^T\theta^{(k+1)}\bigr)_+\\
