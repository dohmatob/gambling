...
\begin{itemize}
\item $A^T$: \quad transpose of the matrix $A$;
\item $\begin{bmatrix}A\\B\end{bmatrix}$: vertical stacking of the matrices $A$ and $B$;
\item $v_j$: \quad $j$th component of the vector $v$;
...
\item \textit{l.s.c}: \quad acronyme for ``lower semi-continuous'';
\item $E^{-1}(U)$: \quad the pre-image of $U \subset \mathbb{R}^n$ under a matrix $E \in \mathbb{R}^{p,n}$,
