...
poker). Our algorithm only performs basic iterations (i.e matvec
multiplications, clipping, etc., and no calls to external first-order
oracles, no matrix inversions, etc.) and is applicable to a broad
class of two-person zero-sum games including simultaneous games and
sequential games with incomplete information and perfect recall. The
...
as a matrix game with convex polytopial profiles. We prove that the
number of iterations needed to produce a Nash equilibrium with a given
precision is inversely proportional to the precision. As
proof-of-concept, we also present experimental results on simulated
and real games (Kuhn poker).
\end{abstract}
...
Let $m$ and $n$ be positive integers. Given two
...
\paragraph*{Sequential games with incomplete
  information: Kuhn poker.} This game is a simplified form of poker
...
\begin{figure}[!htpb]
  ...
  that the proved convergence rate for our algorithm is observed.}
