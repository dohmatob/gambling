...
\subsection{Notation, terminology, and some aspects of modern convex analysis}
...
\paragraph{\textbf{Modern convex analysis.}}
...
\subsubsection{A note about matrix games on simplexes.} It should be
noted
...
\subsubsection{Approximate feasibility and optimality}
...
Let us now, introduce the notion of $\epsilon$-subgradients
...
We develop a $\mathcal{O}(1/\epsilon)$ primal-dual
algorithm (Algorithm \ref{Tab:algo}) for computing Nash $(\epsilon,
  0)$-equilibria in sequential two-person zero-sum games with
...
\item[--] The reported linear convergence rate is not in terms of
  ...
  example the complicated projections $\proj_{Q_k}$ are applied at each iteration.%%  Of
