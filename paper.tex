...
\begin{algorithm}[htb]
  ...
  $d^{(0)} = 0$; $\tau, \sigma > 0 \text{ s.t. }\tau\sigma \|K\|_2^2 < 1$; $k = 0$.\\
  \textbf{Precompute} $\eta_0 \leftarrow \tau A^Ty_0$\\
  \Repeat{
    $d^{(k)} < \epsilon$}{
    \begin{eqnarray*}
      r^{(k)} &\leftarrow& e - E\tilde{x}^{(k)}\\ 
    \zeta^{(k+1)} &\leftarrow& \zeta^{(k)} - \sigma r^{(k)}\\
    x^{(k+1)} &\leftarrow& \bigl(x^{(k)} + \eta_0 + E^T\zeta^{(k+1)}\bigr)_+\\
    \tilde{x}^{(k+1)} &\leftarrow& 2x^{(k+1)} - x^{(k)}\\
    d^{(k+1)} &\leftarrow& max(d^{(k)}, \frac{\|x^{(k+1)} - x^{(k)}\|^2_2}{2\tau} + \frac{\|r^{(k)}\|^2_2}{2})\\
    k &\leftarrow& k + 1
    \end{eqnarray*}
