...
\begin{itemize}
\item $\mathbb{R}^{m, n}$ \quad space of all $m$-by-$p$ real matrices;
...
\item \textit{p.c.l.s.c}: \quad acronym for \textit{proper convex lower semi-continuous};
\item $f^*$: \quad Fenchel transform (a.k.a convex conjugate) of a \textit{p.c.l.s.c} function $f$;
\item $(1 + \sigma \partial f)^{-1}$: resolvent (a.k.a proximal) operator of a \textit{p.c.l.s.c} function $f$, for a given stepsize $\sigma > 0$
\end{itemize}
...
\begin{remark}
  In practice, $A$, $E$, and $F$ are very sparse: $A$ will be sparse because a concrete sequential game will
...
Furthermore, one easily checks that $F^*$ and $G$ are \textit{p.c.l.s.c} with resolvent
