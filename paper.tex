...
%%   \label{eq:opt_pb}
%% \end{eqnarray}
%% where $J(\mathbf{w}) := \zeta\|\mathbf{w}\|_{\ell_{1}} + (1 -
%% \zeta)\|\mathbf{w}\|_{TV}$ is the regularization and $\alpha \ge 0$
%% controls the amount of regularization. The
%% parameter $\zeta \in [0, 1]$, also known as the $\ell_{1}$ \textit{ratio}, is the trade-off between the
...
%% \\
%%     \text{where} \quad
%%     J_{\mu}(\mathbf{w}) :=\zeta \|\mathbf{w}\|_{\ell_{1},\mu} +
%%     (1 - \zeta)\|\mathbf{w}\|_{TV, \mu} \quad
%% \end{align}
