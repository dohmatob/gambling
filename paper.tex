...
a constraint ``$x \in C$'' (thus obtaining a new problem) where $C$ is a convex set onto which euclidean projections
can be easily computed (for example, a probability simplex), then we simply need to replace the operator ``$(.)_+$''
with ``$\Pi_C$'' in the equations to obtain a corresponding algorithm.
\end{remark}
