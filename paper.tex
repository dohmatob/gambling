...
\begin{figure}[!htpb]
  ...
  \caption{Convergence curves of Algorithm
    ...
    value of game. In \textit{(a)}, one can see the linear (i.e
    exponential) of the algorithm in \cite{gilpinfirst}, inbetween
    consecutive breakpoints on the $\epsilon$ grid. The duality gaps
    are computed according to formula \eqref{eq:mg_pd}. The
    first-order smoothing algorithm from \cite{nesterov2005a} jitters around as the
    iterations go on because even the smoothed problem becomes heavily
    ill-conditioned near solutions. On the other hand, our propsed
    algorithm seems to beat both of the aforementioned algorithms, and
  its proven $\mathcal{O}(1/\epsilon)$ convergence rate is clearly verified.}
...
implementing them for such games would require us to compute the
complicated projections $\prox_{Q_k}$.  We recall that
