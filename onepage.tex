...
\begin{algorithm}[htb]
  ...
  \textbf{precompute} $\|K\|$, where $K$ is constructed as in equations \eqref{eq:unconstrained_pb}. This can be done using a \textit{power iteration} on $K^TK$, for example.\\
  \textbf{initialize}
