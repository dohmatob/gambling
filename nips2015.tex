...
Thus the iterates $(y^{(k)},p^{(k)},x^{(k)},q^{(k)})$ generated by
Algorithm \ref{Tab:algo} converge ergodically to a
$(\rho,\epsilon)$-Nash Equilibrium for the GSP \eqref{eq:unconstrained_pb}
in no more than $2d_0\|K\|max(5d_0/\rho, 1/\epsilon)$ steps.
\end{theorem}
