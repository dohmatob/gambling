\documentclass[envcountsame]{llcn2e/llncs}
\usepackage{url}
...
\title{\bf A simple and efficient algorithm for computing approximate
  Nash equilibria in two-person zero-sum games with imcomplete
  information\smallskip\hrule\hrule}
...
algorithm for computing Nash $(\rho, \epsilon$)-equilibria (in a sense
to be made clear later) for
...
subgradient of $f$ at $x$, namely $\{h\in X | f(z) \ge f(x) + \langle
h, z - x\rangle \forall z \in X\}$.

\begin{definition}[$\epsilon$-subgradient]
Given a scalar $\epsilon > 0$ and a function $f:X \subseteq \mathbb{R}^n
\rightarrow [-\infty,+\infty]$, the $\epsilon$-subgradient of $f$ the
set-valued function
\begin{eqnarray}
