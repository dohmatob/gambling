...
with \textit{duality gap} given by
\begin{equation}
...
In section \ref{sec:related_work}, we give a brief overview of existing methods for solving \eqref{eq:approx_pb}. In section \ref{sec:algo} we derive the proposed algorithm for solving this problem and prove its $\mathcal{O}(1/\epsilon)$ convergence rate. With construct with a demonstration on Kuhn 3-card Poker.
...
\label{sec:algo}
We now derive our primal-dual algorithm for solving the Nash equilibrium problem \eqref{eq:primal_pb}. The algorithm is obtained from the general primal-dual Algorithm 39 proposed in \cite{chambolle2010}. Though this scheme has recently gained considerable popularity in the signal processing community
...
\begin{equation}
  \begin{split}
    H^*\left(K\begin{bmatrix}y\\p\end{bmatrix}\right) &= \underset{x,q}{\text{max}}\text{ }\begin{bmatrix}x\\q\end{bmatrix}^TK\begin{bmatrix}y\\p\end{bmatrix} - H(x, q) \\
      &= \underset{x, q}{\text{max}}\text{ }x^T(Ay - E_1^Tp) - i_{\mathbb{R}^{n_1}_+}(x) + q^TE_2y - f^Tq\\
      &= \underset{x \geq 0}{\text{max}}\text{ }x^T(Ay - E_1^Tp) + \underset{q}{\text{max}}\text{ }q^T(E_2y - f)\\
...
As in \textbf{nesterov, c-p, etc.}, we generate a $1000 \times 1000$ random matrix whose entries are uniformly identically distributed in the closed interval $[-1, 1]$.
