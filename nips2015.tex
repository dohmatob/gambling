...
$\langle z, w\rangle$ The components of $z \in \mathbb{R}^n$ will be
...
by $\|z\| := \sqrt{\langle z, z\rangle} = \sqrt{\sum_{j=0}^{n - 1}{z_j^2}}$.
...
\begin{equation}
  \begin{aligned}
     \text{minimize }\langle e_1, p\rangle, \hspace{.5em}\text{ subject to: }(y,p) \in
     \mathbb{R}^{n_2} \times \mathbb{R}^{l_1},  y \ge 0, E_2y = e_2, -Ay +
    E_1^Tp \geq 0.
  \end{aligned}
  \label{eq:primal_pb}
\end{equation}
...
\begin{equation}
  \begin{aligned}
    \text{maximize}-\langle e_2, q\rangle,\hspace{.5em}\text{subject to: }(x,q) \in
    \mathbb{R}^{n_1} \times \mathbb{R}^{l_2},  x \ge 0, E_1x = e_1, A^Tx + E_2^Tq \geq 0.
  \end{aligned}
  \label{eq:dual_pb}
\end{equation}
...
\label{eq:polyhedron}
\end{equation}
Note that $\langle e_1, p\rangle = p_0$ and $\langle e_2, q\rangle = q_0$ by definition of $e_1$ and
...
following equivalent saddle-point form\footnote{Note that in
  \cite{hoda2010smoothing}, the roles of $x$ and $y$ are swapped.}
...
\textit{condition number} for the game. Their ingenious idea was to
show that \eqref{eq:gilpin} can futher be written in the form
\[\mathrm{maximize}\{G(x, y):=
max\{\langle x, Av\rangle - \langle u, Ay\rangle |
(u,v) \in Q_1 \times Q_2|(x,y) \in Q_1 \times Q_2\},
\]
and then show there exists a scalar
$\delta > 0$ such that ``distance between $(x, y)$ and the set of
equilibria'' $\le \dfrac{G(x, y)}{\delta}$,
for any pair of realization plans $(x, y) \in Q_1 \times Q_2$. Their
algorithm is then derived by iteratively applying Nesterov smoothing
with a geometrically decreasing sequence of tolerance levels on
primal-dual gap function $G$. It should be noted however that
\begin{itemize}
  ...
  \begin{eqnarray}
    ...
    \begin{aligned}
      ...
      (-\infty, +\infty], \hspace{1em} g_1(y, p) :=
        i_{\mathbb{R}^{n_2}_+}(y) + \langle e_1,p\rangle\\
        g_2: \mathbb{R}^{n_1} &\times \mathbb{R}^{l_2} \rightarrow
        (-\infty, +\infty],\hspace{1em} g_2(x, q) :=
          i_{\mathbb{R}^{n_1}_+}(x) + \langle e_2, q\rangle
    \end{aligned}
    \right\}
    \label{eq:things}
  \end{eqnarray}
