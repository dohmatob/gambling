...
For each information set $h$ of player $k$, $\sigma_h$ is the sequence of choices from the root node, to --any node of-- $h$, ignoring the choices of the other players. \textit{Perfect recall} means that a player cannot get additional information on the game treee by remembering their earlier choices. $C_h$ is the set of choices at $h$. The set of \textit{sequences} of player $k$, denoted $\mathcal{S}_k$, is defined by 
\begin{equation}
\mathcal{S}_k := \{\emptyset\} \cup \{\sigma_h c\text{ } |\text{ } h \in I_k, c \in C_h\}
\end{equation}
Each element $\sigma$ of $\mathcal{S}_k$ is a path joining the root node to a leaf in the game tree, with the choices of all other players (the other player, and the \textit{chance} player, aka \textit{nature}) deleted. Such a sequence represents a possible sequence of choices made by player $k$ in the game, from start to finish.

$\sigma_k(t)$ is the unique sequence in $\mathcal{S}_k$ which can be extended to the path from root to node $t$, by including the choices of the other players;
...
A realization plan $z$ for a player $k$ induces a \textit{behavioural-strategy} $\beta_k$ defined by
\begin{equation}
...
where $h \in \mathcal{I}_k$ and $c \in \mathcal{C}_h$, and $\beta_k(c)$ is the probability to play $c$ given that the player is at information set $h$. From this definition, it is clear that each behavioural-strategy is equivalent to a mixed-strategy.
...
$A$, an $n_1$-by-$n_2$ matrix, is the \textit{payoff matrix} from player 1's perspective of the game. Formally $A = (A_{\sigma,\tau})$, where
  \begin{equation}
...
The nanuplet $(\mathcal{I}_1, \mathcal{I}_2, \mathcal{S}_1, \mathcal{S}_2, A, E_1, E_2, e_1, e_2)$ completely specifies a game in sequence-form. However, since were are only interested in computing Nash equilibria, the quintuplet $(A, E_1, E_2, e_1, e_2)$ conveys all required information, as we will see in the next section.
...
%% combinations of sequences of choices of the players, will actually lead to a leaf (i.e. end the game);
%% $E$ and $F$ will be sparse because the kinks of possible sequences of choices of each player will
%% zig-zag between only a limited number of the player's information sets so that a choice at an information set will
