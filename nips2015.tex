...
\begin{equation}
  \begin{aligned}
    \text{maximize}-\langle e_2, q\rangle,\hspace{.5em}\text{subject to }(y,q) \in
...
\[\mathrm{maximize}\{G(x, y):=G(x, y) :=
max\{\langle x, Av\rangle - \langle u, Ay\rangle |
(u,v) \in Q_1 \times Q_2|(x,y) \in Q_1 \times Q_2\},
\]
and then show that if $\mathcal{E}$ is the set of Nash equilibria for
the game, then  there exists a scalar
$\delta > 0$ such that $distance((x, y), \mathcal{E}) \le \dfrac{G(x, y)}{\delta}$,
for any pair of realization plans $(x, y) \in Q_1 \times Q_2$. Their
algorithm is then to apply Nesterov smoothing with a geometrically
decreasing sequence of tolerance levels on primal-dual gap function $G$.
