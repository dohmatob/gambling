...
\State  $\lambda \leftarrow 1/\|K\|$, ${v}_a^{(0)} \leftarrow 0$, $k \leftarrow 0$
...
E_1^Tp^{(k+1)}))_+$
\State $\Delta x^{(k+1)} \leftarrow x^{(k+1)}-x^{(k)}$
\State $\Delta q^{(k+1)} \leftarrow q^{(k+1)}-q^{(k)}$
\State  $q^{(k+1)} \leftarrow q^{(k)} + \Delta q^{(k+1)}$
\State $y^{(k+1)} \leftarrow y^{(k+1)} - \lambda (A^T\Delta x^{(k+1)}
+ E_2^T\Delta q^{(k+1)})$
\State $\Delta y^{(k+1)} \leftarrow y^{(k+1)}-y^{(k)}$
\State $p^{(k+1)} \leftarrow p^{(k+1)} + \lambda E_1\Delta x^{(k+1)}$
\State $\Delta p^{(k+1)} \leftarrow p^{(k+1)}-p^{(k)}$
\State ${v}_a^{(k+1)} \leftarrow {v}_a^{(k)} + \frac{1}{\lambda}(\Delta
