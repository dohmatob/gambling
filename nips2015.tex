...
Each point $z \in Q_k$ are called \textit{realization plan} and induces a \textit{behavioural-strategy} $\beta_k$ such that for each information set $h \in \mathcal{I}_k$ the probability for player $k$ to make a choice $c \in \mathcal{C}_h$ from any node in $h$ is
\begin{equation}
